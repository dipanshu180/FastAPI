...
async def login(login_data : UserLoginModel, session:AsyncSession = Depends(get_session)):
    ...
    if user is not None:
        ...
        if password_valid:
            ...
            referesh_token = create_access_token(user_data={"uid": str(user.uid), "email": user.email}, refresh=True,
            expiry=timedelta(days=REFERESH_TOKEN_EXPIRE)
            )
...
async def get_new_access_token(token_details: dict = Depends(RefreshTokenBearer())):
    ...
    # return {"message": "New access token generated"}
    # raise InvalidToken
